...
def create_csv_file(cur_date):
    with open(f"data_{cur_date}.csv", "w", encoding="utf8") as file:
...
def get_disabled(item):
    try:
        product_disabled = item.find("p", class_="disabled").text
    except AttributeError:
        product_disabled = 0
...
def collect_data(pages_count, work_dir, cur_date):
    ...
    for page in range(1, pages_count + 1):
        ...
        for item in items_cards:
            ...
            with open(f"{work_dir}/data_{cur_date}.csv", "a", encoding="utf-8") as file:
                writer = csv.writer(file)
...
def get_discount(cur_date, work_dir):
    ...
    print('Всего найдено товаров со скидкой, в магазине - garfild.by:', len(discount_data), "\n" "--------" "\n")
