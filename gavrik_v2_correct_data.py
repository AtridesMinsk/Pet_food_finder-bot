...
def get_product_name(item):
    product_name = item.find("div", class_="h4").text.strip()
    return product_name
...
def get_new_price(item):
    try:
        product_new_price = item.find("span", class_="spec_price").text.rstrip(
            "  р.")
...
def get_old_price(item):
    try:
        product_old_price = item.find("span", class_="price_old").text.rstrip(
            "  р.")
...
def get_product_url(item):
    product_url = f'{item.find("div", class_="image").find("a", href=True).attrs["href"]}'
    return product_url
...
def collect_data(pages_count, work_dir, cur_date):
    ...
    for page in range(1, pages_count + 1):
        with open(f"{work_dir}/page_{page}.html", "r", encoding="utf-8", newline=None) as file:
            src = file.read()

        soup = BeautifulSoup(src, "lxml")
        items_cards = soup.find_all("div", class_="product-layout product-list col-xs-12")
        ...
        for item in items_cards:
            ...
            with open(f"{work_dir}/data_{cur_date}.csv", "a", encoding="utf8") as file:
                writer = csv.writer(file)
